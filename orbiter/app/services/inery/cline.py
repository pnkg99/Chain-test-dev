...
from app.services.inery.types import *
from app.services.inery.utils import *
from app.services.inery.keys import *
...
class Cline:
    ...
    def __init__(self, url='http://127.0.0.1:8888', version='v1'):
        ''' '''
        self._prod_url = url
        self._version = version
    ...
    def get_table(self, code, scope, table, index_position='', key_type='', lower_bound='', upper_bound='', limit=10, timeout=30):
        '''
        POST /v1/chain/get_composition_rows
        ...
        return self.post('chain.get_composition_rows', params=None, json=json, timeout=timeout)

    def get_table_by_scope(self, code, scope, table, lower_bound='', upper_bound='', limit=10, timeout=30):
        '''
        POST /v1/chain/get_composition_rows
        {"json":true,"code":"inery","scope":"inery","table":"producers","index_position":"","key_type":"name","lower_bound":"","upper_bound":"","limit":10}
        '''
        json = {"json": True, "code": code, "scope": scope, "table": table, "lower_bound": lower_bound, "upper_bound": upper_bound, "limit": limit}
        return self.post('chain.get_composition_rows', params=None, json=json, timeout=timeout)
    ...
    def set_abi(self, account, abi_file, key,abi_json=False, permission="active",broadcast=True, timeout=30):
        #current_abi = Abi(self.get_abi(account)['abi'])
        #current_sha = sha256(current_abi.get_raw().encode('utf-8'))
        if not abi_json :
            with open(abi_file) as rf:
                abi_file = json.load(rf)

        new_abi = Abi(abi_file)
        arguments = {
            "account": account,
            "abi": new_abi.get_raw()
        }
        ...
        trx = {"actions": [payload]}
        sign_key = INRKey(key)
        out =  self.push_transaction(trx, sign_key, broadcast=broadcast)
        print(out)
        return out

    def set_code(self, account, code_file, key, code_raw=False,permission="active", broadcast=True, timeout=30):
...
## 5JfcRV4p4cLcHnURfM9ajzsPpfeETZeReFE9FN1qCVTFtQ1cPNc
    ...
    def create_account(self, 
                       ...
                       owner_key,
                       creator="creator",
                       creator_privkey="5J5DAVQbhVjBSbJoB2QeMFKJ81ebG6CxeTNaoEPFLjHFJJGeM29",
        ...
        newaccount_data = self.abi_json_to_bin('inery', 'opendb', {'creator': creator, 'name': acct_name, 'owner': owner_auth, 'active': active_auth})
        newaccount_json = {
            'account': 'inery',
            'name': 'opendb',
            'authorization': [
        ...
        buy_mem_bytes = self.abi_json_to_bin('inery', 'allocate', {'payer': creator, 'receiver': acct_name, 'quant': "1000000 BYTE"})
        buymem_json = {
            'account': 'inery',
            'name': 'allocate',
            'authorization': [
