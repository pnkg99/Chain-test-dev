            ...
            for line in lines:
                if 'Release:' in line:
                    ...
                    if 18 <= major_version <= 24:
                        Logger.get_instance().info(f'Ubuntu version : {version}')
                        return
                    else:
...
class Chain:
    ...
    def _init_once(self):
        ...
        self.general_path = join_path(self.config_path, 'general.json') # NOTE! should be moved to setup
        ...
        self.accounts = read_file(join_path(self.config_path, 'accounts.json'), True) # NOTE! should be moved to setup
        self.chains = read_file(join_path(self.config_path, 'servers.json'), True) # NOTE! should be moved to setup
        self.general = read_file(self.general_path, True) # NOTE! should be moved to setup

        self.genesis_account = self.accounts['GENESIS_ACCOUNT'] # NOTE! should be moved (or at least edited) to setup
        self.system_accounts = self.accounts['SYSTEM_ACCOUNTS'] # NOTE! should be moved (or at least edited) to setup

        self.master_chain = self.chains['MASTER_CHAIN'] # NOTE! should be moved  (or at least edited)to setup
        self.side_chains = self.chains['SIDE_CHAINS'] # NOTE! should be moved (or at least edited) to setup

        self.genesis_pubkey = self.genesis_account['PUBLIC_KEY'] # NOTE! should be moved (or at least edited) to setup
        self.genesis_json = self.general['GENESIS_JSON'] # NOTE! should be moved (or at least edited) to setup
        self.genesis_json['initial_key'] = self.genesis_pubkey # NOTE! should be moved (or at least edited) to setup

        self.wallet_psw = self.general['WALLET_PASSWORD'] # NOTE! should be moved (or at least edited) to setup
        self.features = self.general['PROTOCOL_FEATURES'] # NOTE! should be moved (or at least edited) to setup
    
    def setup(self, general, accounts, servers):
        print('TO BE DONE!')
    ...
    def run_command(self, command_list, url=None, success_message='', suppress_error_for=None):
        ...
        if success_message:
            Logger.get_instance().info(f'✅ {success_message}')  # \u2705 - backup unicode for check mark
        return out, error
    ...
    def unlock_wallet(self):
        ...
        if error: 
            if 'Already unlocked' in error:
                Logger.get_instance().info('Default wallet is already unlocked.')
            elif 'Invalid wallet password' in error: 
                error_msg('Incorrect wallet password')
            else:
                error_msg(error)
        else:
            Logger.get_instance().info('Unlocked default wallet')
    ...
    def create_key(self):
        out, _ = self.run_command(['create', 'key', '--to-console'])
        
        private = re.search(r'Private key: ([^\n]+)', out).group(1)
        public = re.search(r'Public key: ([^\n]+)', out).group(1)
    ...
    def is_started(self):
        # Checks if the main chain is started        
        try:
            # Look for a nodine process with data-dir in its args
            path = join_path(self.data_path, 'master_chain')
    ...
    def run_cluster(self, path, node):
        ...
        try:
            ...
            Logger.get_instance().info(f'Blockchain {path} already started')
        except subprocess.CalledProcessError:
            # Check for files that indicate a chain was initialized
            if os.path.exists(join_path(path, 'blockchain', 'data', 'blocks')):
                os.system(join_path(path, 'start.sh'))
                Logger.get_instance().info(f'Blockchain {path} started')
                return
            ...
            Logger.get_instance().info(f'✅ Blockchain {path} started')  # \u2705 - backup unicode for check mark
    ...
    def initialize_chain(self, url):
        ...
        Logger.get_instance().info('Blockchain initilized')
        time.sleep(1)
    ...
    def stop_chain(self):
        ...
        for pid in nodine_pids.split():
            ...
            Logger.get_instance().info(f'Terminated process pid={pid}')
            time.sleep(0.2)
    ...
    def remove_chain(self):
        for dir in os.listdir(self.data_path):
            ...
            Logger.get_instance().info(f'Blockchain {dir_path} removed')
    ...
    def generate_script(self, path, node, type):
        ...
        base_command = [
            ...
            '\t--abi-serializer-max-time=100000',
...
if __name__ == '__main__':
    ...
    logger = Logger.get_instance(verbose=args.verbose)
